...
import { Truck, Users, LayoutDashboard, LogOut, Settings, Bell, Route, Wrench, Fuel, ShieldCheck, BarChart3, Moon, Sun } from 'lucide-react';
import Link from 'next/link';
import { usePathname, useRouter } from 'next/navigation';

const managerNavItems = [
    { label: 'Command Center', icon: LayoutDashboard, href: '/manager/command-center' },
    { label: 'Vehicle Registry', icon: Truck, href: '/manager/vehicles' },
    { label: 'Driver Profiles', icon: Users, href: '/manager/drivers' },
    { label: 'Trip Dispatcher', icon: Route, href: '/manager/trips' },
    { label: 'Maintenance Logs', icon: Wrench, href: '/manager/maintenance' },
    { label: 'Expenses & Fuel', icon: Fuel, href: '/manager/expenses' },
    { label: 'Driver Safety', icon: ShieldCheck, href: '/manager/safety' },
    { label: 'Analytics', icon: BarChart3, href: '/manager/overview' },
];
...
export default function ManagerLayout({ children }: { children: React.ReactNode }) {
    ...
    const { isAllowed } = useRoleGuard(Role.MANAGER);

    if (!isAllowed) return null;

    const pageLabel = managerNavItems.find((i) => i.href === pathname)?.label || 'Overview';
            ...
            <aside className="w-64 bg-white dark:bg-neutral-900 border-r border-neutral-200 dark:border-neutral-800 flex flex-col shrink-0 transition-colors duration-300">
                ...
                <div className="h-16 flex items-center gap-3 px-5 border-b border-neutral-200 dark:border-neutral-800">
                    <div className="bg-emerald-500 p-1.5 rounded-lg shrink-0">
                        <Truck className="w-5 h-5 text-white dark:text-neutral-950" />
                    </div>
                ...
                <div className="px-5 py-3 border-b border-neutral-200 dark:border-neutral-800">
                    <span className="text-xs font-semibold uppercase tracking-widest text-emerald-600 dark:text-emerald-500">Fleet Manager</span>
                </div>
                    ...
                    {managerNavItems.map((item) => {
                        ...
                        return (
                            <Link key={item.href} href={item.href}
                                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors group ${isActive ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-medium' : 'text-neutral-600 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-800/60 hover:text-neutral-900 dark:hover:text-neutral-200'}`}>
                                <item.icon className={`w-[18px] h-[18px] shrink-0 ${isActive ? 'text-emerald-600 dark:text-emerald-400' : 'text-neutral-400 dark:text-neutral-500 group-hover:text-neutral-700 dark:group-hover:text-neutral-300'}`} />
                ...
                <div className="p-3 border-t border-neutral-200 dark:border-neutral-800 space-y-0.5">
                    <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-neutral-600 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-800/60 hover:text-neutral-900 dark:hover:text-neutral-200 transition-colors group">
                        <Settings className="w-[18px] h-[18px] shrink-0 text-neutral-400 dark:text-neutral-500 group-hover:text-neutral-700 dark:group-hover:text-neutral-300" />
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            <div className="text-right hidden sm:block">
                                ...
                                <p className="text-xs text-neutral-500 mt-1 uppercase tracking-wider">Fleet Manager</p>
                            </div>
                            <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-emerald-500 to-emerald-400 dark:from-emerald-600 dark:to-emerald-400 flex items-center justify-center text-white dark:text-neutral-950 font-bold text-sm shadow-sm">
