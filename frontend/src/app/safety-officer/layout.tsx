...
import { LogOut, Bell, LayoutGrid, Users, FileText, Truck, Sun, Moon } from 'lucide-react';
import Link from 'next/link';
import { usePathname, useRouter } from 'next/navigation';

const navItems = [
    { label: 'Overview', icon: LayoutGrid, href: '/safety-officer' },
    { label: 'Driver Safety', icon: Users, href: '/safety-officer/drivers' },
    { label: 'Compliance', icon: FileText, href: '/safety-officer/compliance' },
];
...
export default function SafetyLayout({ children }: { children: React.ReactNode }) {
    ...
    const { isAllowed } = useRoleGuard(Role.SAFETY_OFFICER);
        ...
        <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-neutral-200 font-sans flex text-sm transition-colors duration-300">
            ...
            <aside className="w-64 bg-white dark:bg-neutral-900 border-r border-neutral-200 dark:border-neutral-800 flex flex-col shrink-0 transition-colors duration-300">
                ...
                <div className="px-5 py-3 border-b border-neutral-200 dark:border-neutral-800">
                    <span className="text-xs font-semibold uppercase tracking-widest text-amber-600 dark:text-amber-500">Safety Officer</span>
                </div>
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <Link key={item.href} href={item.href}
                                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors group ${isActive ? 'bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 font-medium' : 'text-neutral-600 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-800/60 hover:text-neutral-900 dark:hover:text-neutral-200'}`}>
                                <item.icon className={`w-[18px] h-[18px] shrink-0 ${isActive ? 'text-amber-600 dark:text-amber-400' : 'text-neutral-400 dark:text-neutral-500 group-hover:text-neutral-700 dark:group-hover:text-neutral-300'}`} />
                                <span className="whitespace-nowrap">{item.label}</span>
                                {isActive && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-amber-500 dark:bg-amber-400" />}
                            </Link>
                        );
                    })}
                </nav>
            ...
            <main className="flex-1 flex flex-col min-w-0">
                <header className="h-16 bg-white/80 dark:bg-neutral-950/80 backdrop-blur-md border-b border-neutral-200 dark:border-neutral-800 flex items-center justify-between px-8 sticky top-0 z-10 transition-colors duration-300">
                    <div className="flex items-center gap-2 text-sm">
                        <span className="text-neutral-500">Safety Officer</span>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <button className="relative p-2 text-neutral-500 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white transition-colors rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800">
                            ...
                            <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-amber-500 rounded-full border-2 border-white dark:border-neutral-950" />
                        </button>
                        ...
                        <div className="flex items-center gap-3">
                            <div className="text-right hidden sm:block">
                                ...
                                <p className="text-xs text-neutral-500 mt-1 uppercase tracking-wider">Safety Officer</p>
                            </div>
                            <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-amber-500 to-amber-400 dark:from-amber-600 dark:to-amber-400 flex items-center justify-center text-white dark:text-neutral-950 font-bold text-sm shadow-sm">
