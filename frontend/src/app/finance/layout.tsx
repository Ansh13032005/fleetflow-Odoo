...
import { LogOut, Bell, LayoutGrid, Fuel, BarChart3, FileText, Truck, Sun, Moon } from 'lucide-react';
import Link from 'next/link';
import { usePathname, useRouter } from 'next/navigation';

const navItems = [
    { label: 'Overview', icon: LayoutGrid, href: '/finance' },
    { label: 'Expenses', icon: Fuel, href: '/finance/expenses' },
    { label: 'Analytics', icon: BarChart3, href: '/finance/analytics' },
...
export default function FinanceLayout({ children }: { children: React.ReactNode }) {
    ...
    const { isAllowed } = useRoleGuard(Role.FINANCIAL_ANALYST);
        ...
        <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-neutral-200 font-sans flex text-sm transition-colors duration-300">
            ...
            <aside className="w-64 bg-white dark:bg-neutral-900 border-r border-neutral-200 dark:border-neutral-800 flex flex-col shrink-0 transition-colors duration-300">
                ...
                <div className="px-5 py-3 border-b border-neutral-200 dark:border-neutral-800">
                    <span className="text-xs font-semibold uppercase tracking-widest text-purple-600 dark:text-purple-400">Financial Analyst</span>
                </div>
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <Link key={item.href} href={item.href}
                                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors group ${isActive ? 'bg-purple-50 dark:bg-purple-500/10 text-purple-600 dark:text-purple-400 font-medium' : 'text-neutral-600 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-800/60 hover:text-neutral-900 dark:hover:text-neutral-200'}`}>
                                <item.icon className={`w-[18px] h-[18px] shrink-0 ${isActive ? 'text-purple-600 dark:text-purple-400' : 'text-neutral-400 dark:text-neutral-500 group-hover:text-neutral-700 dark:group-hover:text-neutral-300'}`} />
                                <span className="whitespace-nowrap">{item.label}</span>
                                {isActive && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-purple-500 dark:bg-purple-400" />}
                            </Link>
                        );
                    })}
                </nav>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            <div className="text-right hidden sm:block">
                                ...
                                <p className="text-xs text-neutral-500 mt-1 uppercase tracking-wider">Financial Analyst</p>
                            </div>
                            <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-purple-500 to-purple-400 dark:from-purple-600 dark:to-purple-400 flex items-center justify-center text-white font-bold text-sm shadow-sm">
